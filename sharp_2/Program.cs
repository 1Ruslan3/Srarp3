...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        int maxPatients = 5; // Максимальное количество мест в приемном отделении
        int doctorCount = 3; // Количество врачей
        int maxConsultationTime = 10; // Максимальное время консультации

        // Создание экземпляра инфекционного отделения
...
class Doctor
{
    ...
    public async Task ConsultPatient(Patient patient, int maxConsultationTime)
    {
        ...
        await Task.Delay(consultationTime * 1000); // Симуляция времени консультации
...
class InfectiousDiseaseWard
{
    ...
    {
        // Запуск задач для каждого врача
        ...
        // Запуск симуляции прихода пациентов
        while (true)
        {
            // Симуляция прихода пациента
            ...
            await Task.Delay(1000); // Симуляция времени между прибытием пациентов
        }
    }
    ...
    private async Task DoctorWork(Doctor doctor)
    {
        while (true)
        {
            ...
            {
                await Task.Delay(500); // Если нет пациентов, ждем немного перед следующей проверкой
            }
        }
    }
    ...
    {
        // Случайным образом определяем, больной ли пациент
